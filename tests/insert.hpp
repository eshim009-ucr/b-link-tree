...
TEST(InsertTest, LeafNode) {
	...
	EXPECT_EQ(mem_read(root, memory).values[0], 2);
	...
	EXPECT_EQ(mem_read(root, memory).values[1], 3);
	dump_node_list(log_stream, memory);
	ASSERT_TRUE(is_unlocked(root, log_stream, memory));
	...
	EXPECT_EQ(mem_read(root, memory).values[1], 1);
	EXPECT_EQ(mem_read(root, memory).keys[2], 5);
	EXPECT_EQ(mem_read(root, memory).values[2], 3);
	dump_node_list(log_stream, memory);
	ASSERT_TRUE(is_unlocked(root, log_stream, memory));
...
TEST(InsertTest, SplitRoot) {
	...
	value = 0;
	EXPECT_EQ(insert(&root, -value, value, memory), SUCCESS);
	dump_node_list(log_stream, memory);
	EXPECT_EQ(mem_read(root, memory).keys[0], -value);
	EXPECT_EQ(mem_read(root, memory).values[0], value);
	ASSERT_TRUE(is_unlocked(root, log_stream, memory));
	value = -5;
	EXPECT_EQ(insert(&root, -value, value, memory), SUCCESS);
	dump_node_list(log_stream, memory);
	EXPECT_EQ(mem_read(root, memory).keys[1], -value);
	EXPECT_EQ(mem_read(root, memory).values[1], value);
	ASSERT_TRUE(is_unlocked(root, log_stream, memory));
	value = -3;
	EXPECT_EQ(insert(&root, -value, value, memory), SUCCESS);
	dump_node_list(log_stream, memory);
	...
	EXPECT_EQ(insert(&root, -value, value, memory), SUCCESS);
	dump_node_list(log_stream, memory);
	lchild = mem_read(root, memory).values[0];
	EXPECT_EQ(mem_read(lchild, memory).keys[1], -value);
	EXPECT_EQ(mem_read(lchild, memory).values[1], value);
	ASSERT_TRUE(is_unlocked(root, log_stream, memory));
	value = -4;
	EXPECT_EQ(insert(&root, -value, value, memory), SUCCESS);
	dump_node_list(log_stream, memory);
